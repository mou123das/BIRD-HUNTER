...
a = ["1. To control the Ninja use the arrow keys (left and right).",
     ...
     "    the red line.",
     "5. You will win the game if you kill all the birds."
     ]
...
def game_state(a):
    ...
    if a=="Game Over":
        game_state_font = pygame.font.Font('SNAP.TTF', 100)
        b=290
    else:
        game_state_font = pygame.font.Font('SNAP.TTF', 120)
        b=307

    sm_font = pygame.font.Font('GOUDOSB.TTF', 40)
    sm_text = sm_font.render(t, True, dark_red)

    game_state_text = game_state_font.render(a, True, black)
    ...
    while True:
        ...
        screen.blit(game_state_text, (100, 150))  # Displaying the required text
        screen.blit(sm_text, (140, b))  # Displaying the message
        display_score(270, 367, 45, black)  # Displaying the final score
...
def has_Collision_Ocurred(BirdX, BirdY, RockX, RockY):
    ...
    distance = math.sqrt(math.pow(BirdX - RockX, 2) + (
        math.pow(BirdY - RockY, 2)))  # calculating the distance between specific bird and the rock

    if distance < 45:  # if distance is less than 34 return True else False
        return True
    else:
        return False
...
def gameloop():
    ...
    # Here we are working with 8 Birds and so the properties are defined using lists
    ...
    BirdX_speed = []  # List containing the rates at which birds would move in +X direction
    BirdY_speed = []  # List containing the rates at which birds would move in +Y direction
    num_of_birds = 8  # The number of birds
    ...
    while True:
        ...
        for i in range(num_of_birds):
            ...
            # Game State checking, whether game is over or the person has won
            if BirdY[i] > 390:  # If any of the birds crosses the violet line
                game_state("Game Over")  # Call game_state() menu

            if score_count == num_of_birds:  #If all the 8 birds are killed
                game_state("You Win!")  # Call game_state() menu

            BirdX[i] += BirdX_speed[i]  # Moving the Birds

            if BirdX[i] <= 0:  # if any of the birds reach the left boundary
                BirdX_speed[i] = 2  # now we will move it towards right
                ...
                BirdX_speed[i] = -2  # now we will move it towards left
                BirdY[i] += BirdY_speed[i]  # and lower it down by BirdY_speed value
            ...
            if collision == True:  # If a collision has occured
                ...
                BirdY[i]=-1000    # Moving it out of the screen
